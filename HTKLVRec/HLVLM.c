...
FSLM_ngram *CreateBoNGramLV (MemHeap *heap, int vocSize, int counts[NSIZE]);
NEntry *GetNEntryLV (FSLM_ngram *nglm, LMId ndx[NSIZE], Boolean create);
                           ...
                           Vocab *vocab)
{
   ...
   for (i = 1; i <= count; ++i) {
      ...
      ne = GetNEntryLV (nglm,  ndx+1, FALSE);
      if (ne == NULL)
...
FSLM *ReadARPALM(MemHeap *heap, char *lmfn, Vocab *vocab)
{
   ...
   /* PreLMla  lm->data.nglm = CreateBoNGramLV (heap, nng[1], nng); */
   lm->data.nglm = CreateBoNGramLV (heap, vocab->nprons, nng);
...
LogFloat LMTransProb_ngram (FSLM *lm, LMState src, PronId pronid, LMState *dest)
{
   ...
   if (src == (Ptr) 0xffffffff) {
      ...
      *dest = (LMState) GetNEntryLV (nglm, hist, FALSE);
      return 0.0;
   }
   ...
   if (!src) {          /* unigram case */
      ...
      if (se)        /* found */
         ...
         for ( ; l > 0; --l) {
            ...
            ne = GetNEntryLV (nglm, hist, FALSE);
            /* try to find pronid in SEntry array */
            se = FindSEntry (ne->se, pronid, 0, ne->nse - 1);
            assert (!se || (se->word == pronid));
            if (se) { /* found it */
   ...
   if (pronid != lm->endPronId) {
      ...
      ne = (LMState) GetNEntryLV (nglm, hist, FALSE);
      for ( ; !ne && (l > 0); --l) {
         hist[l] = 0;              /* back off */
         ne = (LMState) GetNEntryLV (nglm, hist, FALSE);
      }
...
LogFloat LMLookAhead_ngram (FSLM *lm, LMState src, PronId minPron, PronId maxPron)
{
   ...
   if (neSrc) {
      ...
      for (l = hiIdx - 1; l >= 0; --l) {
         ...
         hist[l+1] = 0;
         ne[l] = GetNEntryLV (nglm, hist, FALSE);
         assert (ne[l]);
   ...
   for (hash = 0; hash < nglm->hashsize; hash++) {
      for (ne = nglm->hashtab[hash]; ne; ne=ne->link) {
         ...
         if (hiIdx == NSIZE) {  /* unigram */
            ...
            ne->nebo = GetNEntryLV (nglm, hist, FALSE);
            assert (ne->nebo);
         }
      }
   }
...
/* EXPORT->GetNEntryLV: Access specific NGram entry indexed by ndx */
NEntry *GetNEntryLV (FSLM_ngram *nglm, LMId ndx[NSIZE], Boolean create)
{
   NEntry *ne;
   unsigned int hash;
...
/* EXPORT->CreateBoNGramLV: Allocate and create basic NGram structures */
FSLM_ngram *CreateBoNGramLV (MemHeap *heap, int vocSize, int counts[NSIZE])
{
   LMId ndx[NSIZE];
   int i,k;
