...
static Boolean GetThisMacroCount(char *macroname, char type, int *count) {
    ...
    /*if (strncasecmp(AUTOMACRONAMEPREFIX, charptr, strlen(AUTOMACRONAMEPREFIX)) != 0)
        return FALSE;*/
    if (strncmp(AUTOMACRONAMEPREFIX, charptr, strlen(AUTOMACRONAMEPREFIX)) != 0) 
        return FALSE;
...
static Boolean ParseBooleanValue(char *strval) {
    ...
    for (i = 0; i < strlen(strval); ++i)
        buf[i] = toupper(strval[i]);
    buf[i] = '\0';

    if (strcmp(buf, "TRUE") == 0 || strcmp(buf, "T") == 0)
        return TRUE;
    else if (strcmp(buf, "FALSE") == 0 || strcmp(buf, "F") == 0)
        return FALSE;
    else
