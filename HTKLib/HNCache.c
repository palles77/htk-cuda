...
/* do knuth shuffling */
static void KnuthShuffle(void *list, size_t stPos, size_t edPos, int unitLen) {
...
static unsigned int GetLog2Ceil(unsigned int val) {
...
static void QuickNetShuffle(void *list, int stPos, int edPos, int unitLen) {
    int i, n;
...
/* shuffle a segment */
static void ShuffleSegment(void *list, size_t stPos, size_t edPos, int unitLen) {
    switch (shuffKind) {
        case KNUTHFSK:
        case KNUTHRSK:
...
static void UpdateFrmPtr(DataCache *cache, FrmIndex *frmPtr) {
    int uttIdx;
...
static void CopyExtFrame2Batch(UttElem *uttElem, int curIdx, FELink feaElem, NFloat *dstPtr) {
    int i, srcIdx;
    float *srcPtr;
#ifdef DOUBLEANN
    int j;
...
static Boolean FillBatchFRM(DataCache *cache, int nSamples) {
    int i;
...
static Boolean FillBatchUTT(DataCache *cache, int nSamples, int *uttCnt) {
    int i, uttIdx, frmIdx;
    UttElem *uttElem;
    Boolean finish = FALSE;
...
static Boolean FillBatchPLUTT(DataCache *cache, int nSamples) {
...
/* cz277 - many */
static int GetDataFromCache(DataCache *cache, FELink feaElem, int offset) {
...
static int GetAugFeaFromCache(DataCache *cache, FELink feaElem, int offset) {
    int i, j, uttIdx;
    NFloat *feaPtr;
    UttElem *uttElem;
...
static void GetHardLabelFromCache(DataCache *cache, int tgtDim, NFloat *labBat, int offset) {
...
static int GetFeatureLabelFromCache(DataCache *cache, int tgtDim, NFloat *labBat, int offset) {
    int i, uttIdx, frmIdx;
    NFloat *labPtr;
    UttElem *uttElem;
    float *flabPtr;
