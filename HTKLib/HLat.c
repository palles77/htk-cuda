...
char *hlat_vc_id = "$Id: HLat.c,v 1.2 2015/12/18 18:18:18 xl207 Exp $";
...
*/
static LogFloat  LatLMTrans (LModel *lm, LMState src, LabId wordId, LMState *dest, LMState *src_mix, LMState *dest_mix)
{
   LogFloat prob;
   ...
   if (wordId == nullWord) {
      ...
      wordId = startLMWord;
      prob = LMTrans (lm, src, wordId, dest, src_mix, dest_mix);
      return 0.0;
   }
...
*/
static SubLNode *FindAddSubLNode (MemHeap *heap, LNode *ln, LMState lmstate, LMState *lmstate_mix, int *nsln)
{
   SubLNode *subln;
   
   for (subln = (SubLNode *) ln->hook; subln; subln = subln->next) {
      /* checking both ngram LM state AND weight set state */
      if (subln->data.lmstate == lmstate && subln->lmstate_mix[0] == lmstate_mix[0]) 
         return subln;
   }
   if (!subln) {
      ++*nsln;
...
Lattice *LatExpand (MemHeap *heap, Lattice *lat, LModel *lm)
{
   ...
   */
   ...
   FindAddSubLNode (&slnHeap, LatStartNode (lat), NULL, src_mix, &nsln);
   ...
   for (i = 0; i < lat->nn; ++i) {
      ...
      for (startSLN = (SubLNode *) ln->hook; startSLN; startSLN = startSLN->next) {
         ...
         for (la = ln->foll; la; la = la->farc) {
            assert (la->start == ln);
            lmprob = LatLMTrans (lm, startSLN->data.lmstate, la->end->word->wordName, &dest, startSLN->lmstate_mix, dest_mix);
            endSLN = FindAddSubLNode (&slnHeap, la->end, dest, dest_mix, &nsln);
            ...
            sla->lmprob = lmprob;
#else
            sla->lmprob = (intpltLatGram) ? ( (logintLatGram) ? (wgtLatGram * la->lmlike + (1.0 - wgtLatGram) * lmprob) :
                                              LOG_NATURAL( wgtLatGram * UNLOG_NATURAL(la->lmlike) + (1.0 - wgtLatGram) * UNLOG_NATURAL(lmprob) ) ) : lmprob; 
#endif
            sla->end = endSLN;
            sla->la = la;
