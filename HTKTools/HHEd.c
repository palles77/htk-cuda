...
/* Rank-based mixture splitting (MU) from Evermann et al ICASSP 2005 paper */
static float heaviestScale = 1.0;      /* scaling to select the heviest component */
static float minMixUpOcc = 0.0;        /* Minimum occupancy to permit a split */
...
static Boolean rankMixUp = FALSE; /* use rank-based mixture splitting*/
...
void SetConfParms(void)
{
   ...
   if (nParm>0) {
      ...
      /* rank-based mixture splitting*/
      if (GetConfBool(cParm,nParm,"RANKMIXUP",&b)) rankMixUp = b;
      if (GetConfFlt(cParm,nParm,"MINMIXUPOCC",&f)) minMixUpOcc = f;
   }
}
...
int main(int argc, char *argv[])
{
   ...
   CreateHeap(&hmmHeap,"Model Heap",MSTAK,1,1.0,400000,4000000);
...
/* HeaviestRankMix: rank-based version of the above from 
Evermann et al. ICASSP 2005 paper.
*/
int HeaviestRankMix(char *hname, MixtureElem *me, int M, float occ)
{
   float omax,w,gThresh,max=LZERO;
   int m,maxm,omaxm,j;
   MixPDF *mp;
   Vector rankg, rankw;
   Vector vecg, vecw;
   ...
   gThresh = meanGC - 4.0*stdGC;
   /* Original scheme to split the components */
   omaxm = 1;  mp = me[1].mpdf;
   omax = me[1].weight - 1000*(long int)mp->hook;
   if ((long int)mp->hook < 5 && mp->gConst < gThresh) {
      omax -= 5000.0; mp->hook = (void *)(5+(long int)mp->hook);
      HError(-2637,"HeaviestRankMix: mix 1 in %s has v.small gConst [%f]",
             hname,mp->gConst);
   }
   for (m=2; m<=M; m++) {   
      mp = me[m].mpdf;
      w = me[m].weight - 1000*(long int)mp->hook;
      if ((long int)mp->hook < 5 && mp->gConst < gThresh) {
         w -= 5000.0; mp->hook = (void *)(5+(long int)mp->hook);
         HError(-2637,"HeaviestRankMix: mix %d in %s has v.small gConst [%f]",
                m,hname,mp->gConst);
      }
      if (w>omax) {
         omax = w; omaxm = m;
   ...
   for (m=1; m<=M; m++) {   
      ...
      w = me[m].weight;
      if ((long int)mp->hook < 5 && mp->gConst < gThresh) {
         mp->hook = (void *)(5+(long int)mp->hook);
         HError(-2637,"HeaviestRankMix: mix %d in %s has v.small gConst [%f]",
                m,hname,mp->gConst);
      }
      if ((occ<0) || (w*occ > minMixUpOcc)) { 
         /* Since in the log-domain hard to ensure repeated slit does not occur */
         w = heaviestScale*log(w) + mp->gConst  - 1000*(long int)mp->hook;;
         if (w>max || maxm==0) {
            max = w; maxm = m;
         }
      }
   }
   rankg = CreateVector(&gstack,M);
   rankw = CreateVector(&gstack,M);
   vecw = CreateVector(&gstack,M);
   vecg = CreateVector(&gstack,M);
   for (m=1; m<=M; m++) {   
      mp = me[m].mpdf;
      w = me[m].weight;
      rankg[m]=0;
      ...
      vecg[m]=mp->gConst;
      for (j=1;j<m;j++) {
         if (mp->gConst > vecg[j]) rankg[j]=rankg[j]+1;
         else rankg[m] = rankg[m]+1;
         if (w > vecw[j]) rankw[j]=rankw[j]+1;
         else rankw[m] = rankw[m]+1;
      }
   }
   maxm = 0;
   for (m=1; m<=M; m++) {   
      ...
      mp = me[m].mpdf;
      if ((occ<0) || (w*occ > minMixUpOcc)) { 
         /* Ensure repeated slit does not occur */
         w = -rankw[m] - rankg[m]  - 1000*(long int)mp->hook;
         if (w>max || maxm==0) {
            max = w; maxm = m;
         }
      }
   }
   FreeVector(&gstack,rankg);
   if (maxm>0) {
      if (maxm != omaxm) {
         printf("Split component %d (%f %f %f) instead of %d (%f %f)\n",maxm,max,me[maxm].weight,me[maxm].mpdf->gConst,omaxm,omax,me[omaxm].mpdf->gConst);
      } else {
         printf("Split component %d (%f %f %f) same as %d (%f %f)\n",maxm,max,me[maxm].weight,me[maxm].mpdf->gConst,omaxm,omax,me[omaxm].mpdf->gConst);
      }
      if (me[maxm].weight<=MINMIX)
         HError(2697,"HeaviestRankMix:  heaviest mix is defunct!");
      if (trace & T_DET) {
         printf("               : Split %d (weight=%.3f, count=%ld, score=%.3f)\n",
                maxm, me[maxm].weight, (long int)me[maxm].mpdf->hook, max);
         fflush(stdout);
      ...
      printf("No component above threshold\n");
   }
   return maxm;
}
...
void UpMix(char *hname, StreamElem *ste, int oldM, int newM)
{
   ...
   while (count<newM) {
      if (rankMixUp) {
      	 if (occStatsLoaded && (minMixUpOcc > 0)) /* use minimum occupancy threshold */
            m = HeaviestRankMix(hname,me,count,(long int)ste->hook);
      	 else 
	   m = HeaviestRankMix(hname,me,count,(long int)-1);
      	 if (m>0) {
            ++count;
...
void FixDefunctMix(char *hname,StreamElem *ste, int n)
{
   ...
   for (count = 0; count<n; ++count) {
      ...
      if (rankMixUp){
      	 if (occStatsLoaded && (minMixUpOcc > 0)) /* use minimum occupancy threshold */
            m = HeaviestRankMix(hname,me,M,(long int)ste->hook);
      	 else 
	   m = HeaviestRankMix(hname,me,M,(long int)-1);
      	 if (m>0) {
            SplitMix(me+m,&m1,&m2,vSize);
            me[m] = m1; me[l] = m2;
      	 } else {
...
/* cz277 - direct2 */
Boolean GetTheOnlyOneHMM(HMMSet *hset, HLink *oneHMM) {
    int h, hmmcnt = 0;
    MLink m;
      ...
      for (q=tmpSet->mtab[h]; q!=NULL; q=q->next) 
         if (q->type=='l') {
            if ((p=FindMacroName(hset,'l',q->id))==NULL) {
               if (GetTheOnlyOneHMM(hset, &oldHMM) == FALSE)	/* cz277 - direct2 */
...
/* Give any component below threshold a very large penalty */

void SetStreamElemOcc(HMMSet *hset) 
{
   HMMScanState hss;
   StateInfo *si   ;
   StreamElem *ste;
   ...
   while(GoNextState(&hss,FALSE)) {
      ...
      while (GoNextStream(&hss,TRUE)) {
         ...
         if (M<0) M=-M;
         for (m=1;m<=M;m++) { 
            wgt = ste->spdf.cpdf[m].weight;
            if (wgt*occ < minMixUpOcc) {
               cnt++; 
            }
         }
      }
   ...
   EndHMMScan(&hss);
   ClearSeenFlags(hset,CLR_ALL);
   printf("Occupancy penalised %d components at %f\n",cnt,minMixUpOcc); 
}
...
void MixUpCommand(void)
{
   ...
   SetGCStats();
   if (rankMixUp && occStatsLoaded && (minMixUpOcc > 0)) SetStreamElemOcc(hset);
   for (i=ilist; i!=NULL; i=i->next) {
      ste = (StreamElem *)i->item;
      M = ste->nMix;
      if (M>0) ste->nMix=-M;
...
static int SetGMMStreamWidth(char *invoker, int s, int n) {
    int nedit = 0;
    char c = ' ';
    HMMScanState hss;
    HLink hmm = NULL;
    ...
    if (!(hss.isCont || (hss.hset->hsKind == TIEDHS))) 
        HError(2640, "%s: Can only resize continuous and tied systems", invoker);
    ...
    while(GoNextMix(&hss, FALSE)) {
        ...
        case FULLC:
            ...
            break;
        default:
            HError(2640, "%s: Can only resize DIAGC or FULLC", invoker);
        }
        if (trace & (T_SIZ | T_DET)) {
            if (nedit == 0) {
                printf(" For model.state[");
                hmm = NULL;
            }
            if (hmm != hss.hmm) {
                printf("]\n  %12s.state", hss.mac->id->name);
                c='[';
                hmm = hss.hmm;
            }
            printf("%c%d", c, hss.i);
...
void SetStreamWidthCommand(void)
{
   ...
   /* cz277 - direct2 */
   nedit = SetGMMStreamWidth("SetStreamWidthCommand", s, n);
...
void PowerSizeCommand(void)
{
   ...
   /* cz277 - bug fix */
   if (occStatsLoaded && (minMixUpOcc > 0)) SetStreamElemOcc(hset);
...
void BorrowOneANN(char *invoker, ADLink annDef, HMMSet *srcset, HMMSet *dstset) {
    int i, j;
    LELink layerElem;
    AILink annInfo, curInfo;

    /*  */
    BorrowOneANNMacro(invoker, 'N', annDef, srcset, dstset);
    /*  */
    if (dstset->annSet == NULL)
        InitANNSet(dstset);
    /*  */
    ...
    for (i = 0; i < annDef->layerNum; ++i) {
        ...
        BorrowOneANNMacro(invoker, 'L', layerElem, srcset, dstset);
        if (layerElem->feaMix != NULL)
            BorrowOneANNMacro(invoker, 'F', layerElem->feaMix, srcset, dstset);
        if (layerElem->wghtMat != NULL)
            BorrowOneANNMacro(invoker, 'M', layerElem->wghtMat, srcset, dstset);
        if (layerElem->biasVec != NULL)
            BorrowOneANNMacro(invoker, 'V', layerElem->biasVec, srcset, dstset);
        if (layerElem->actfunVecs != NULL)
            for (j = 0; j <= layerElem->actfunParmNum; ++i)
                if (layerElem->actfunVecs[j] != NULL)
                    BorrowOneANNMacro(invoker, 'V', layerElem->actfunVecs[j], srcset, dstset);
    }
}
...
/* -------------- Linearly Transform ANN Layers -------------- */

/* chaopig */
/* cz277 - gmml: temporary function */
Vector LoadMVNvector(char *path, char *type) {
    Source src;
    char buf[MAXSTRLEN];
    int dim;
    Vector vec;

    if (InitSource(path, &src, NoFilter) < SUCCESS)
        HError(9999, "did not find file %s to load", path);

    SkipComment(&src);
    ReadString(&src, buf);
    if (strcmp(buf, "<CEPSNORM>") != 0)
        HError(9999, "<CEPSNORM> expected");
    ReadString(&src, buf);
    
    while (strcmp(buf, type) != 0 && feof(src.f) == FALSE)
        ReadString(&src, buf);

    if (strcmp(buf, type) != 0)
        HError(9999, "%s expected", type);

    ReadInt(&src, &dim, 1, FALSE);
    vec = CreateVector(&gcheap, dim);
    if (!ReadVector(&src, vec, FALSE))
        HError(9999, "Load vector failed");
...
/* cz277 - gmml */
void LinearXFormOneANNLayer() {
    char buf[MAXSTRLEN], xformPath[MAXSTRLEN], macroname[MAXSTRLEN];
    char srctype;
    int i, j, k;
    InputXForm *inputxform;
    AdaptXForm *adaptxform;
    MLink srcmacro;
    LabId srclabid;
    Matrix xformmat = NULL, outmat;
    Vector xformvec = NULL, outvec;
    LELink layerElem = NULL;
    /* for m/v norm */
    Vector meanvec = NULL, varvec = NULL;
    char meanPath[MAXSTRLEN], varPath[MAXSTRLEN];

    /*  */
    ChkedAlpha("Next keyword", buf);
    if (strcmp(buf, "<INPUTXFORM>") == 0) {
        ChkedAlpha("Input linear transform path", xformPath);
        inputxform = LoadInputXForm(NULL, NameOf(xformPath, macroname), xformPath);
        if (inputxform == NULL)
            HError(2600, "LinearXFormOneANNLayer: Load the input xform failed");
        xformmat = inputxform->xform->xform[1];
        xformvec = inputxform->xform->bias;
    }
    else if (strcmp(buf, "<ADAPTXFORM>") == 0) {
        ChkedAlpha("Adapt transform path", xformPath);
        ChkedAlpha("Adapt macro name", macroname);
        adaptxform = LoadOneXForm(hset, macroname, xformPath);
        xformmat = adaptxform->xformSet->xforms[1]->xform[1];
        xformvec = adaptxform->xformSet->xforms[1]->bias;
    }
    else if (strcmp(buf, "<CEPSNORM>") == 0) {
        ChkedAlpha("Mean vector path", meanPath);
        meanvec = LoadMVNvector(meanPath, "<MEAN>");
        ChkedAlpha("Variance vector path", varPath);
        varvec = LoadMVNvector(varPath, "<VARIANCE>");
    }
    else
        HError(2650, "LinearXFormOneANNLayer: Unsupported keyword %s", buf);
    /*  */
    ChkedAlpha("Next keyword", buf);
    if (strcmp(buf, "<SOURCEMACRO>") == 0) {
        ChkedAlpha("Source macro type", buf);
        if (buf[0] != '~' || strlen(buf) != 2)
            HError(2650, "LinearXFormOneANNLayer: ~? macro type expected");        
        srctype = buf[1];
        ChkedAlpha("Source macro name", buf);
        srclabid = GetLabId(buf, FALSE);
        if (srclabid == NULL || (srcmacro = FindMacroName(hset, srctype, srclabid)) == NULL)
            HError(2679, "CopyANNParameters: Cannot find the source macro ~%c \"%s\"", srctype, srclabid->name);
        layerElem = (LELink) srcmacro->structure;
    }
    else
        HError(2650, "LinearXFormOneANNLayer: Unsupported keyword %s", buf);
    ...
    if (strcmp(buf, "<INSIDE>") == 0)
        HError(9999, "LinearXFormOneANNLayer: <INSIDE> function not implemented yet");
    else if (strcmp(buf, "<OUTSIDE>") == 0) {
        /* TODO: check dimensions here: */
        outmat = CreateMatrix(&gcheap, layerElem->nodeNum, layerElem->inputDim);
        outvec = CreateVector(&gcheap, layerElem->nodeNum);

        if (xformmat != NULL) {
            /* TODO: change to a better math function */
            for (i = 1; i <= NumRows(xformmat); ++i) {
                for (j = 1; j <= layerElem->inputDim; ++j) {
                    outmat[i][j] = 0.0;
                    for (k = 1; k <= NumCols(xformmat); ++k) 
                        outmat[i][j] += xformmat[i][k] * 
                                        layerElem->wghtMat->variables->matElems[(k - 1) * layerElem->inputDim + (j - 1)];
                }
                outvec[i] = 0.0;
                for (j = 1; j <= NumCols(xformmat); ++j)
                    outvec[i] += xformmat[i][j] * 
                                 layerElem->biasVec->variables->vecElems[j - 1];
            }
            if (xformvec != NULL)
                for (i = 1; i <= VectorSize(xformvec); ++i)
                    outvec[i] += xformvec[i];

            /*CopyMatrix2NMatrix(outmat, layerElem->wghtMat->variables);
            CopyVector2NVector(outvec, layerElem->biasVec->variables);
#ifdef CUDA
            SyncNMatrixHost2Dev(layerElem->wghtMat->variables); 
            SyncNVectorHost2Dev(layerElem->biasVec->variables);
#endif
            
            Dispose(&gcheap, outvec);
            Dispose(&gcheap, outmat);*/
        }
        else if (meanvec != NULL && varvec != NULL) {
            for (i = 1; i <= layerElem->nodeNum; ++i) {
                for (j = 1; j <= layerElem->inputDim; ++j) {
                    outmat[i][j] = layerElem->wghtMat->variables->matElems[(i - 1) * layerElem->inputDim + (j - 1)];
                    outmat[i][j] /= varvec[i];
                }
            }
            for (i = 1; i <= layerElem->nodeNum; ++i) {
                outvec[i] = (layerElem->biasVec->variables->vecElems[i - 1] - meanvec[i]);
                outvec[i] /= varvec[i];
            }
        }
        else
            HError(2650, "LinearXFormOneANNLayer: Function not supported yet %s", buf);
        /*  */
        CopyMatrix2NMatrix(outmat, layerElem->wghtMat->variables);
        CopyVector2NVector(outvec, layerElem->biasVec->variables);
        /*  */
...
/* -------------- Merge Linear Activation Functions -------------- */

/* cz277 - gmml */
void MergeAffineActivationFunction() {
...
static char *cmdmap[] = {"AT","RT","SS","CL","CO","JO","MU","TI","UF","NC",
                         ...
                         "AV", "SV", "AM", "SM", "AF", "IL", "CF", "CD", 
                         "EL", "CA", "EF", "CP", "DL", "CM", "CH", "SL", 
                         "LX", "MA", "" };
...
typedef enum           { AT=1, RT , SS , CL , CO , JO , MU , TI , UF , NC ,
                         ...
                         LS , QS , TB , TR , AU , GQ , MD , ST , LT ,
                         MM , DP , HK , FC , FA , FV, XF, PS, PR, 
                         AV , SV , AM , SM , AF , IL, CF, CD, EL, CA, 
                         EF, CP, DL, CM, CH, SL, LX, MA }
cmdNum;
...
void DoEdit(char * editFn)
{
   ...
   for (;;) {
      ...
      case LX: LinearXFormOneANNLayer(); break;			/* cz277 - gmml */
      case MA: MergeAffineActivationFunction(); break;		/* cz277 - gmml */
      default: 
